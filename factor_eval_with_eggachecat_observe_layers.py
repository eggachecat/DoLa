...
from dola_with_eggachecat_observe_layers import DoLa
...
if __name__ == "__main__":
    ...
    early_exit_layers = list(range(1,33))
    ...
    # factory, data_collection, clean_data_collection_fn = make_data_collector_factory()

    for name, module in llm.model.named_modules():
        print(name)

    llm.model = pv.IntervenableModel([
        {
            "component":  f"model.layers.{layer - 1}.input",
    ...
    answers = []
    result_dict = {'is_correct': [], 'model_answer': [], 'model_completion': [], 'full_input_text': [], 'modle_true_outputs': [], 'modle_false_outputs': []}
    for sample in tqdm(list_data_dict):
        ...
        generate_kwargs = dict(max_new_tokens=args.max_new_tokens, top_p=args.top_p, top_k=args.top_k, temperature=args.temperature, repetition_penalty=args.repetition_penalty, mode=mode, mature_layer=mature_layer, premature_layer=premature_layer, candidate_premature_layers=candidate_premature_layers, relative_top=args.relative_top, relative_top_value=args.relative_top_value, data_collection_factory=data_collection_factory)
